    ...
    if XQFunction.isStrNil(wanType)
        ...
        if wanType == "pppoe" then
            ...
            if XQFunction.isStrNil(pppoeName) or XQFunction.isStrNil(pppoePwd) then
                ...
                if mtu and not XQLanWanUtil.checkMTU(mtu) then
                    ...
                    if not XQLanWanUtil.setWanPPPoE(pppoeName, pppoePwd, dns1, dns2, peerDns, mtu, special) then
                        code = 1529
                    end
                end
            end
        ...
        elseif wanType == "static" then
            ...
            if not LuciDatatypes.ipaddr(staticIp) then
                code = 1530
            elseif not LuciDatatypes.ipaddr(staticMask) then
                code = 1531
...
function qosSwitch()
    ...
    if not switch then
        ...
        os.execute("/etc/init.d/traffic restart")
    end
...
function setWifiApMode()
    ...
    if ssid and (password or encryption == "NONE") then
        local ap = XQAPModule.setWifiAPMode(ssid, encryption, enctype, password, channel, bandwidth)
        if not ap.scan then
            result.code = 1617
