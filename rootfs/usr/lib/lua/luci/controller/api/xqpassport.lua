...
function index()
    ...
    entry({"api", "xqpassport", "login"}, call("passportLogin"), (""), 401, true)
    entry({"api", "xqpassport", "userInfo"}, call("getUserInfo"), (""), 402)
    --entry({"api", "xqpassport", "accounts"}, call("getAccounts"), (""), 403, true)
    --entry({"api", "xqpassport", "search"}, call("doSearch"), (""), 404)
    entry({"api", "xqpassport", "rigister"}, call("routerRegister"), (""), 405, true)
    entry({"api", "xqpassport", "binded"}, call("getBindInfo"), (""), 406, true)
    ...
    if password ~= nil then
        ...
        if login and login.code == 0 then
            if XQSysUtil.getPassportBindInfo() then
                ...
                if unbound and (tonumber(unbound.code) == 0 or tonumber(unbound.code) == 3001) then
...
function getAccounts()
    ...
    local result = {}
    result["code"] = 0
    result["accounts"] = XQDBUtil.fetchAccounts()
    LuciHttp.write_json(result)
end
...
function doSearch()
    local XQNetUtil = require("xiaoqiang.util.XQNetUtil")
    local uuid = LuciHttp.formvalue("uuid") or ""
    local query = LuciHttp.formvalue("query")
    local page = tonumber(LuciHttp.formvalue("page")) or 1
    local result = XQNetUtil.searchUser(uuid,query,page)
    if result then
        result["code"] = 0
        result["page"] = page
    else
        result["code"] = 1540
        result["msg"] = XQErrorUtil.getErrorMessage(1540)
    end
    LuciHttp.write_json(result)
end
