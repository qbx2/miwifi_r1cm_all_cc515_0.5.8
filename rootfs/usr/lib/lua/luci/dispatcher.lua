...
function dispatch(request)
    ...
    local remoteaddr = http.getenv("REMOTE_ADDR")
    if track.sysauth and track.noauth~=true and remoteaddr ~= "127.0.0.1" then
    ...
    elseif remoteaddr == "127.0.0.1" and track.sysauth and track.noauth~=true then
        if check_show_syslock(track.sysauth_authenticator) then
            return
        end
    end
...
-- @param   noauth this node need not auth (optional)
