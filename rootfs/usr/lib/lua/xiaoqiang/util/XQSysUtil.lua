...
function diskExist()
    ...
    return not LuciFs.access(XQConfigs.DISK_CHECK_PATH)
end
...
function setRouterName(routerName)
    ...
    --[[
    if getBindUserInfo() and (oldRouterName ~= routerName or getRouterNamePending() == "1") then
        XQLog.log(7,"Update Router Name...")
        local result = XQNetUtil.updateDeviceName(nil,routerName)
        if result and type(result) == "table" and tonumber(result.code) == 0 then
            XQLog.log(7,"Update Router Name Succeed")
            setRouterNamePending('0')
        else
            XQLog.log(3,"Update Router Name Failed:",result)
