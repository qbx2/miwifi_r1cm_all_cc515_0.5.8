...
-- 3: 功能冲突由于DMZ开启
function setPortForward(name, ip, sport, dport, proto, force)
    if not XQFunction.isStrNil(ip) and _portCheck(tonumber(sport)) and _portCheck(tonumber(dport)) then
        ...
        if _portConflictCheck(sport) and not force then
            return 2
        end
...
-- 1: 参数不合法
-- 2: 端口冲突(设置的端口和已有端口有重叠)
function setRangePortForward(name, ip, fport, tport, proto, force)
