...
config6855Esw()
{
	...
	if [ "$1" = "LLLLW" ]; then
		...
		echo "switch setup for DMZ"
		# change from mode LLLLW
		#set PVID
		#switch reg w <port-reg> <PVID, 10003 for pvid 3>
		switch reg w 2014 10003 #port0, dmz port, vlan 3, port near usb port
		switch reg w 2114 10001 #port1
		switch reg w 2214 10001 #port2
		switch reg w 2314 10001 #port3
		...
		switch vlan set 0 1 01110111 #lan port, vlan 1
		switch vlan set 1 2 00001011 #wan port, vlan 2, port near DC
		switch vlan set 2 3 10000011 #dmz port, vlan 3, port near usb port
		#switch vlan dump
	fi
...
setup_switch()
{
	...
	dmzinterface="$(uci get network.dmz.ifname 2>/dev/null)"
	if [ "$mode" = "AP" ]; then
		restore6855Esw
	elif [ "$dmzinterface" = 'eth0.3' ]; then
		#DLLLW
		config6855Esw DMZ
	else
